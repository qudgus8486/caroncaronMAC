...
//global
int node_index;
float stop_deviation,deviation,left_curv, right_curv;
...
void trafficCallBack(const darknet_ros_msgs::BoundingBoxes::ConstPtr& detection)
{
    for(int i = 0 ; i < detection->bounding_boxes.size(); i++){
        ...
        if(traffic_flag[detection->bounding_boxes[i].Class][3]>500){
...
double cross(int signal){
    if(stop_deviation<10){
        if(signal==GOSTRAIGHT){
            if(beyes_GRX[0]>0.8)  speed_reduction.data = 1.0;
            else if((beyes_GRX[1]>0.8) && stop_deviation<4) speed_reduction.data = 0.0;
            else if((beyes_GRX[1]>0.8)) speed_reduction.data = 0.3;
        }

        else if(signal==TURNLEFT){
            if(beyes_TL[1]>0.8)  speed_reduction.data = 1.0;
            else if((beyes_TL[1]<0.2) && stop_deviation<4) speed_reduction.data = 0.0;
            else if((beyes_TL[1]<0.2)) speed_reduction.data = 0.3;
        }
        else if(signal==TURNRIGHT){
            speed_reduction.data = 0.5;
        }
    }
    return speed_reduction.data;
}
...
void bayes_traffic()
{
        ...
        if(traffic_flag[9][0]<0.2 && beyes_TL[0]<0.95){
            ...
            tx=beyes_TL[0]*0.3;
            turn_left=beyes_TL[1]*0.7;
            beyes_TL[0]=turn_left/(turn_left+tx);
            beyes_TL[1]=tx/(turn_left+tx);
        }


        if(traffic_flag[0][0]<0.2 && traffic_flag[1][0]<0.2){
        G=beyes_GRX[0]*0.4;
        R=beyes_GRX[1]*0.4;
        X=beyes_GRX[2]*0.2;
